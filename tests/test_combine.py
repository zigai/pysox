...
class TestCombineDefault(unittest.TestCase):
    ...
    def test_build_with_vols(self):
        expected_result = True
        actual_result = self.cbn.build(
            [INPUT_WAV, INPUT_WAV], OUTPUT_FILE, "mix", input_volumes=[0.5, 2]
        )
        self.assertEqual(expected_result, actual_result)
    ...
    def test_failed_build(self):
        ...
        with self.assertRaises(SoxError):
            cbn.build([INPUT_FILE_INVALID, INPUT_WAV], OUTPUT_FILE, "concatenate")
...
class TestCombineTypes(unittest.TestCase):
    ...
    def test_concatenate(self):
        expected = True
        actual = self.cbn.build([INPUT_WAV, INPUT_WAV], OUTPUT_FILE, "concatenate")
        self.assertEqual(expected, actual)
    ...
    def test_mixpower(self):
        expected = True
        actual = self.cbn.build([INPUT_WAV, INPUT_WAV], OUTPUT_FILE, "mix-power")
        self.assertEqual(expected, actual)

    def test_multiply(self):
        expected = True
        actual = self.cbn.build([INPUT_WAV, INPUT_WAV], OUTPUT_FILE, "multiply")
        self.assertEqual(expected, actual)
...
class TestSetInputFormat(unittest.TestCase):
    ...
    def test_build_lesser_len(self):
        cbn = new_combiner()
        cbn.set_input_format(rate=[44100, 44100], channels=[1, 1])
        actual = cbn.build([INPUT_WAV, INPUT_WAV, INPUT_WAV], OUTPUT_FILE, "mix")
        expected = True
        self.assertEqual(expected, actual)
    ...
    def test_build_lesser_len_vol(self):
        ...
        cbn.set_input_format(rate=[44100, 44100], channels=[1, 1])
        actual = cbn.build(
            [INPUT_WAV, INPUT_WAV, INPUT_WAV], OUTPUT_FILE, "mix", input_volumes=[1, 2]
        )
        expected = True
        self.assertEqual(expected, actual)
...
class TestValidateFileFormats(unittest.TestCase):
    ...
    def test_different_num_channels(self):
        with self.assertRaises(IOError):
            combine._validate_file_formats([INPUT_WAV, INPUT_WAV3], "concatenate")
...
class TestBuildInputFormatList(unittest.TestCase):
    def test_none(self):
        expected = [["-v", "1"], ["-v", "1"]]
        actual = combine._build_input_format_list([INPUT_WAV, INPUT_WAV], None, None)
        self.assertEqual(expected, actual)
    ...
    def test_greater_num_fmt(self):
        actual = combine._build_input_format_list(
            [INPUT_WAV, INPUT_WAV], None, [["-t", "wav"], ["-t", "aiff"], ["-t", "wav"]]
        )
        expected = [["-v", "1", "-t", "wav"], ["-v", "1", "-t", "aiff"]]
        self.assertEqual(expected, actual)
    ...
    def test_lesser_num_fmt(self):
        actual = combine._build_input_format_list(
            [INPUT_WAV, INPUT_WAV, INPUT_WAV], None, [["-t", "wav"], ["-t", "aiff"]]
        )
        expected = [["-v", "1", "-t", "wav"], ["-v", "1", "-t", "aiff"], ["-v", "1"]]
        self.assertEqual(expected, actual)
